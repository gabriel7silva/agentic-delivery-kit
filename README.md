...
| [`AGENTS.md`](AGENTS.md) | The entry point for an agent asked to read, set up or use this repository: three requests, the setup protocol, the rules that bind it, the map. A pointer file per runtime — `CLAUDE.md`, `GEMINI.md`, `.cursor/rules/pact.mdc`, `.github/copilot-instructions.md`, `.windsurf/rules/pact.md` and thirteen more, generated by `automation/scripts/entry_points.py` — lands on it | Every agent runtime; whoever clones the kit |
